...
public class NameWordBuilder {
   ...
   public void nameFileParser( ) throws JsonParseException, IOException {
      ...
      try {
          ...
          * ?> pair = it.next(); bow.write( pair.getKey() + "|" + pair.getValue() ); bow.newLine(); if (
          * counts.containsKey( pair.getValue() ) ) { counts.put( ( Integer ) pair.getValue(), counts.get(
          * pair.getValue() ) + 1 ); } else { counts.put( ( Integer ) pair.getValue(), 1 ); } } // console(
          * counts.toString() );
          * 
          * // Outputs counts to a file Iterator<Entry<Integer, Integer>> ct = counts.entrySet().iterator();
          * BufferedWriter out = fileWriter( new File( "c:/io/mc2_name_words_counts.csv" ) ); while ( ct.hasNext() ) {
          * Map.Entry<?, ?> nvpr = ct.next(); out.write( nvpr.getKey() + "|" + nvpr.getValue() ); out.newLine(); }
          */
         // end
      } catch ( Exception e ) {
         e.printStackTrace();
