...
public class GeoDNA {
  ...
  public static double[] normalise( double lat, double lon ) {
    return new double[] { GeoDNA.mod( ( lat + 90.0 ), 180.0 ) - 90.0, GeoDNA.mod( ( lon + 180.0 ), 360.0 ) - 180.0, };
  }
  ...
  public static double[] addVector( String geodna, double dy, double dx ) {
    ...
    return new double[] { GeoDNA.mod( ( bits[0] + 90.0 + dy ), 180.0 ) - 90.0,
  ...
  public static String pointFromPointBearingAndDistance( String geodna, double bearing, double distance, int precision ) {
    ...
    double lat1 = bits[0];
    double lon1 = bits[1];
    double lat2 = Math.asin( Math.sin( lat1 ) * Math.cos( distance ) / RADIUS_OF_EARTH ) + Math.cos( lat1 )
        * Math.sin( distance / RADIUS_OF_EARTH ) * Math.cos( bearing );
    double lon2 = lon1
        + Math.atan2( Math.sin( bearing ) * Math.sin( distance / RADIUS_OF_EARTH ) * Math.cos( lat1 ), Math
            .cos( distance / RADIUS_OF_EARTH )
  ...
  public static double distanceInKm( String ga, String gb ) {
    ...
    double x = ( Math.toRadians( b[1] ) - Math.toRadians( a[1] ) )
        * Math.cos( ( Math.toRadians( a[0] ) + Math.toRadians( b[0] ) ) / 2 );
  ...
  public static ArrayList<String> reduce( List<String> geodna_codes ) {
    ...
    for ( int i = 0; i < geodna_codes.size(); i++ ) {
      ...
      if ( codes.get( code ) != null ) {
        ...
        if ( codes.get( parent + 'a' ) == 1 && codes.get( parent + 't' ) == 1 && codes.get( parent + 'g' ) == 1
